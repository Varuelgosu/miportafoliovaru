# Semana 1 — Fundamentos de la Tecnología Web

## 🧠 Temas aprendidos y temas acordados

### Requisitos y Recursos Tecnológicos de la Asignatura
- Uso de **Microsoft Teams**, **WhatsApp**, y **GitHub** con sitio web publicado (Portafolio electrónico).
- Responsabilidad del estudiante: entregar laboratorios y exámenes a tiempo, trabajar en grupo y disponer de laptop e internet propio.

![Imagen1](/content/images/semanas/imagen1.png)

### Portafolio Electrónico
- Crear un sitio web en **GitHub Pages** para documentar el aprendizaje.
- Estructura general:
  - Portada (presentación personal)
  - Información "Sobre mí"
  - Proyectos concluidos
  - Cuaderno (Semana 1 → 16)
  - Reflexión final y bibliografía.

![Imagen2](/content/images/semanas/imagen2.png)

### Fundamentos de la Tecnología Web
- La **tecnología web** forma parte de la evolución digital y permite crear aplicaciones que funcionan en navegadores.
- Existen dos tipos principales de tecnologías de desarrollo:
  - **Software Nativo**
  - **Tecnología Web** 

![Imagen3](/content/images/semanas/imagen3.png)

### Open Web Platform – Web Abierta
- Evita el control de una sola empresa.  
- Promueve transparencia y documentación abierta.  
- Se basa en estándares de organizaciones como **W3C**, **ECMA International** y **IETF**.  
- Tecnologías incluidas: HTML5, CSS, SVG, MathML, WAI-ARIA, ECMAScript, WebGL, etc. 

### Estándares Internacionales para el Desarrollo Web
**Principales:**
- **HTTP/HTTPS** – Protocolo de transferencia de hipertexto.  
- **HTML** – Lenguaje de marcado para definir contenido.  
- **CSS** – Hojas de estilo en cascada.  
- **XML** – Intercambio de información estructurada.  
- **JavaScript** – Lenguaje de programación del navegador 

**Complementarios:**
- **Web APIs**, **SVG**, **WebGL**, **MathML**, y **ARIA** (accesibilidad) 

### DNS – Sistema de Nombres de Dominio
- Traduce los nombres de dominio (por ejemplo `www.uncp.edu.pe`) en direcciones IP para que el navegador pueda acceder a los recursos 

![Imagen5](/content/images/semanas/imagen5.png)

### FrontEnd y BackEnd
- **Frontend:** interfaz y experiencia del usuario (HTML, CSS, JavaScript).  
- **Backend:** lógica, bases de datos y servidores (Python, Java, PHP, Node.js, Ruby, MySQL, PostgreSQL, MongoDB) 

![Imagen4](/content/images/semanas/imagen4.jpg)

### Diseño UX/UI y Figma
- **UX:** experiencia del usuario.  
- **UI:** diseño de la interfaz visual con herramientas como **Figma**, Adobe XD, Gliffy o Balsamiq 

![Imagen9](/content/images/semanas/imagen9.png)

---

## 🧪 Ejercicios de laboratorio

**Práctica de Laboratorio 01: Manejo de Visual Studio Code**
1. Reconocimiento del entorno de VSC.  
2. Uso de la paleta de comandos y **GitHub Codespaces**.  
3. Práctica con comandos `git init`, `add`, `commit`, `push`, y manejo de ramas.  
4. Uso de **Emmet** para generar código rápido.  
5. Atajos de teclado en VSC.

![Imagen12](/content/images/semanas/imagen12.png)

![Imagen10](/content/images/semanas/imagen10.png)

---

## 💭 Reflexión

**¿Qué aprendí?**  
Aprendí los fundamentos de la tecnología web, sus estándares internacionales y la diferencia entre frontend y backend.  
También comprendí la importancia de la Open Web Platform y de usar herramientas abiertas como HTML, CSS y JavaScript.

**¿Cómo aprendí?**  
Mediante el análisis de las diapositivas, la exposición del docente y la práctica en Visual Studio Code.  
Explorando comandos git y usando Emmet pude entender mejor el flujo de trabajo moderno en el desarrollo web.